...
ddtw_function.argtypes = [ctypes.POINTER(ctypes.c_double * 3), ctypes.c_int, ctypes.c_int, ctypes.c_int, ctypes.c_int]
...
wnd = 5  # Gaussian Smoother for the raw signal data
wnd_d = 1  # Gaussian Smoother for the 1st derivative data

delta_thrd_tol = [35] * num 
amp_thrd_tol = [1.2] * num
...
def detectMag():
    ...
    global LastRAW_tol, LastRAW_x, LastRAW_y, LastRAW_z, last_index
    try:
        while True:
            arduino.flushInput()
            flag = 1    
            ...
            # k = 0
            if(len(magnet_info) > 0 and n - magnet_info[0][0] == 25):
                ...
                for j in range(len(tmp_ind)):
                    ...
                    # if flag_:
                    print("tmp_n: ", tmp_n)
                    amp_x = sx[tmp_index][tmp_n-25:tmp_n+25]
                    amp_y = sy[tmp_index][tmp_n-25:tmp_n+25]
                    amp_z = sz[tmp_index][tmp_n-25:tmp_n+25]
                    ...
                    ang_gran = 36
                    dis_gran = 5
                    points = ctypes.c_int(20)
                    axis = ctypes.c_int(3)
                    # 调用C函数
                    start_time = datetime.datetime.now()
                    res = ddtw_function(c_array, ang_gran, dis_gran, points, axis)
                    fin_angle[i] = res
                    end_time = datetime.datetime.now()
                    # print("DDTW time: ", (end_time - start_time).microseconds)
                    # continue
                    ...
                    offset_d_s1 = localize_function(double_array, double_array_2, theta, tmp_index, tmp_sec_index, h, l, sol)

                    print("offset_d_s1: ", offset_d_s1)
                    info.append(float(offset_d_s1))
                    

                    if info[1] - last_index > 20:
                        if tag_info == []:
                            ...
                            for tag_i in range(len(tag_info)):
                                if abs(tag_info[tag_i][1]- info[1]) < 20 and abs(tag_info[tag_i][3] - info[3]) < 20: 
                                    flag_tag = True
                                    ind_tag = tag_i
                                    break
                            # if flag_tag:
            ...
            if(len(tag_info) == 3):
                ...
                # x_axis = [tag_info[0][3], tag_info[1][3], tag_info[2][3]]
